...
本地发布候选和商店素材已准备，开发描述文件已经创建并通过 Debug 实包签名读回，Production CloudKit Schema 已部署并读回；隔离 App ID 下的真实 iPhone ↔ Mac Production 新增、修改和删除墓碑闭环已通过。尚未形成可上传的发布签名归档。没有修改线上页面或 App Store Connect、没有创建或调整发布描述文件、没有上传构建、没有提交审核。
...
- Production 关键双端闭环已通过；系统推送、完整离线冲突、账号切换、通知、生物识别和 TestFlight 候选仍未验证。
